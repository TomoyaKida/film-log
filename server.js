  ...
  db.get(query, [containerId], (err, log) => {
        ...
        db.all('SELECT * FROM lenses', [], (err, lenses) => {
          ...
          res.send(`
            ...
            <body style="font-family:sans-serif; padding:20px; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #333;">📷 新規フィルム装填 [${containerId}]</h2>
    ...
    // パターンB: 未完了のステータスが存在する場合 ➔ ステータス更新画面
      ...
      db.get('SELECT camera_name FROM cameras WHERE camera_id = ?', [log.camera_id], (err, camera) => {
        ...
        // shooting（撮影中）: 「使用終了（撮り切り）」ボタン画面
        if (log.status === 'shooting' || log.status === 'using') {
          actionBtn = `<form action="/action/update" method="POST">
            ...
            <button style="width:100%; padding:12px; background:#dc3545; color:white; border:none; border-radius:5px; font-size:16px; cursor:pointer;">使用終了（撮り切り）</button>
          </form>`;
        } 
        // finished（撮影終了）: 「現像出し」ボタン画面
        else if (log.status === 'finished') {
          actionBtn = `<form action="/action/update" method="POST">
            <input type="hidden" name="log_id" value="${log.log_id}">
            <input type="hidden" name="next_status" value="developing">
            <button style="width:100%; padding:12px; background:#ffc107; color:black; border:none; border-radius:5px; font-size:16px; cursor:pointer;">現像出し</button>
          </form>`;
        } 
        // developing（現像中）: 「現像完了」ボタン画面
        else if (log.status === 'developing') {
          actionBtn = `<form action="/action/update" method="POST">
            <input type="hidden" name="log_id" value="${log.log_id}">
            <input type="hidden" name="next_status" value="completed">
            <button style="width:100%; padding:12px; background:#0d6efd; color:white; border:none; border-radius:5px; font-size:16px; cursor:pointer;">現像完了</button>
          </form>`;
        }
        ...
        res.send(`
          ...
          <body style="font-family:sans-serif; padding:20px; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #333;">🎞️ 進行中のフィルム [${containerId}]</h2>
            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #dee2e6;">
              <p style="margin: 5px 0;"><strong>銘柄:</strong> ${log.film_brand} (ISO ${log.iso_speed || '未設定'})</p>
              <p style="margin: 5px 0;"><strong>カメラ:</strong> ${cameraName}</p>
              <p style="margin: 5px 0;"><strong>現在のステータス:</strong> <span style="background:#e9ecef; padding:2px 8px; border-radius:4px; font-weight:bold;">${log.status}</span></p>
            </div>
            ${actionBtn}
          </body>
          </html>
...
app.post('/action/start', (req, res) => {
  ...
  // status を 'shooting' に変更して保存
  db.run(
    `INSERT INTO film_logs (container_id, camera_id, lens_id, film_brand, iso_speed, status, started_at) VALUES (?, ?, ?, ?, ?, 'shooting', ?)`,
